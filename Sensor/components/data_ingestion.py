...
class DataIngestion:
    ...
    def initiate_data_ingestion(self)->artifact_entity.DataIngestionArtifact:
        try:
            ...
            df:pd.DataFrame = utilis.get_collection_as_data_frame(
            ...
            #Split the dataset directory folder if not available
            ...
            #Save train and test df to feature store folder
            train_df.to_csv(path_or_buf=self.data_ingestion_config.train_file_path, index=False, header=True)
            test_df.to_csv(path_or_buf=self.data_ingestion_config.test_file_path, index=False, header=True)

            #Prepare artifact
            logging.info(f"Preparing the Data Ingestion artifact")
            data_ingestion_artifact = artifact_entity.DataIngestionArtifact(
                feature_store_file_path=self.data_ingestion_config.feature_store_file_path, 
                train_file_path = self.data_ingestion_config.train_file_path, 
                test_file_path = self.data_ingestion_config.test_file_path)

            logging.info(f"The Data Ingestion Artifact is :-> {data_ingestion_artifact}")
